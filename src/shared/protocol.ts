...
export interface ExecuteRequestMessage {
  ...
  readonly payload: HttpRequestDef;
}
...
/**
 * Push the webview's dirty state to the host so the panel title can show
 * a modified marker and the host can warn the user if the panel is closed
 * with unsaved changes.
 *
 * Fire and forget — the host does not reply.
 */
export interface WebviewSetDirtyMessage {
  readonly type: 'webview:set-dirty';
  readonly correlationId: CorrelationId;
  readonly payload: {
...
/**
 * Host → Webview: ask the webview to open a new empty request tab (no file
 * on disk yet). The user fills the form and clicks Save — at that point
 * the webview prompts for a path and the host performs `request:save-request`
 * with the chosen name. Folder prefix in the path auto-creates the folder.
 *
 * Fire and forget.
 */
export interface NewRequestEventMessage {
  readonly type: 'event:new-request';
  readonly correlationId: CorrelationId;
}
...
export interface ExecuteResponseMessage {
  ...
  readonly correlationId: CorrelationId;
  readonly payload: HttpResponseDef;
}
...
export interface CollectionLoadedMessage {
  ...
  readonly correlationId: CorrelationId;
  readonly payload: CollectionTree;
}
...
/**
 * Host asks the webview to open a new empty request tab (no file on disk
 * yet). The user fills the form and clicks Save — at that point the
 * webview prompts for a path (folder prefix auto-creates the folder).
 *
 * Fire and forget.
 */
export interface NewRequestEventMessage {
